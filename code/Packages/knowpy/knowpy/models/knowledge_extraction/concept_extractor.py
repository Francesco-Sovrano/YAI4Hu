...
class ConceptExtractor(ModelManager):
	...
	def get_concept_dict_from_span(span, prevent_verb_lemmatization=False, hidden_dep_list=[]):
		...
		return {
			'span': tuple(span),
			'text': ConceptExtractor.get_concept_text(span),
	...
	def is_core_concept(token):
		...
		return (dep =='ROOT' and token.pos_=='NOUN') or re.search(ConceptExtractor.CONCEPT_REGEXP, dep)

	def get_concept_list_by_doc(self, doc_id, processed_doc, annotation_dict):
		core_concept_list = [
			token
	...
	def get_concept_list(self, doc_parser: DocParser):
		print('Extracting concept list from corpus..')
		...
		return sum(map(lambda x: self.get_concept_list_by_doc(*x), tqdm(doc_spacyd_annotation_iter)), [])
