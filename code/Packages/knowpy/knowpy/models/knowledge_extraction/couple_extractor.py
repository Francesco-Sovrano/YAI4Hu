...
class CoupleExtractor(CE):
	...
	@staticmethod
	def get_grammatical_connection(core, other_core, core_set, avoid_jumps=False): # can be one per core in core_set
	...
	def get_couple_list_by_concept_list(self, concept_list):
		# print('Extracting concept list..')
		core_concept_dict = {}
		...
		# print('Extracting core predicate dict..')
	...
	def get_couple_list(self, doc_parser: DocParser):
		print('Extracting couple list from corpus..')
		...
		doc_spacyd_annotation_iter = list(zip(doc_iter, spacy_doc_iter, annotation_iter))
		return sum(map(lambda x: self.get_couple_list_by_concept_list(self.get_concept_list_by_doc(*x)), tqdm(doc_spacyd_annotation_iter)), [])
