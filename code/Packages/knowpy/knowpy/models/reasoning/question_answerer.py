...
from knowpy.misc.levenshtein_lib import remove_similar_labels, labels_are_similar
...
class QuestionAnswerer(ModelManager):
	...
	def __init__(self, graph, model_options, query_concept_classifier_options, answer_classifier_options, answer_summariser_options, betweenness_centrality):
		# nltk.download('wordnet')
		...
		betweenness_centrality = dict(filter(lambda x:x[-1]>0 and len(self.source_dict_no_equiv[x[0]])>1 and not is_common_word(explode_concept_key(x[0]).lower()), betweenness_centrality.items()))
		if self.log:
		...
		self.sentence_classifier = SentenceClassifier(answer_classifier_options) if answer_classifier_options else None
		# Summarisation
		self.sentence_summariser = NeuralSentenceSummariser(answer_summariser_options) if answer_summariser_options else None
	...
	def get_label(self, concept_uri, explode_if_none=True):
		if concept_uri in self.label_dict:
			return min(self.label_dict[concept_uri], key=len)
	...
	def find_answers_in_concept_graph(self, query_list, concept_uri, question_answer_dict, answer_pertinence_threshold=0.55, add_external_definitions=False, add_clustered_triples=False, include_super_concepts_graph=False, include_sub_concepts_graph=False, consider_incoming_relations=False, tfidf_importance=None):
		assert self.sentence_classifier is not None, "Missing sentence_classifier!"
...
################################################################################################################################################
	...
	def ask(self, question_list, query_concept_similarity_threshold=0.55, answer_pertinence_threshold=0.55, with_numbers=True, remove_stopwords=False, lemmatized=False, keep_the_n_most_similar_concepts=1, add_external_definitions=False, add_clustered_triples=False, include_super_concepts_graph=True, include_sub_concepts_graph=True, consider_incoming_relations=True, tfidf_importance=None, filter_extra_fn=no_wh_words):
		...
		print('Extracting concepts from question_list..')
		...
		for concept_label, concept_count_dict in concepts_dict.items():
			for concept_similarity_dict in itertools.islice(unique_everseen(concept_count_dict["similar_to"], key=lambda x: x["id"]), min(1,keep_the_n_most_similar_concepts)):
				...
				print(f'Extracting answers related to {concept_uri}..')
				concept_query_list = [
	...
	def get_concept_overview(self, query_template_list, concept_uri, concept_label=None, answer_pertinence_threshold=0.3, add_external_definitions=True, add_clustered_triples=True, include_super_concepts_graph=True, include_sub_concepts_graph=True, consider_incoming_relations=True, tfidf_importance=None, filter_extra_fn=no_wh_words):
		# set consider_incoming_relations to False with concept-centred generic questions (e.g. what is it?), otherwise the answers won't be the sought ones
