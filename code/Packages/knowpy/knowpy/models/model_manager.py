...
class ModelManager():
	...
	def __init__(self, model_options=None):
		...
		self.__spacy_model = model_options.get('spacy_model', 'en_core_web_sm')
	...
	def load_sbert_model(sbert_model):
		print(f"## Loading sentence_transformers model <{sbert_model['url']}>, cuda support {sbert_model['use_cuda']}...")
		return SentenceTransformer(sbert_model['url'], device='cpu' if not sbert_model['use_cuda'] else 'cuda')
	...
	def run_tf_embedding(self, doc_list, norm=None, as_question=False):
		def fetch_fn(missing_queries):
			...
			print(f'TF: Embedding {len(batch_list)} batches..')
			if len(batch_list) > 1:
	...
	def run_sbert_embedding(self, inputs, convert_to_tensor=False):
		def fetch_fn(missing_inputs):
			...
			print(f'Modelling {len(missing_inputs)} sentences in {len(batch_list)} batches..')
			# print(np.array(sbert_model['pipeline'](batch_list[0]))[:,0,:].shape)
			print(f'SBert: Embedding {len(batch_list)} batches..')
			if len(batch_list) > 1:
				batched_embeddings = [
					sbert_model.encode(batch, convert_to_tensor=convert_to_tensor)
					for batch in tqdm(batch_list)
				]
				embeddings = np.concatenate(batched_embeddings, 0)
			else:
				embeddings = sbert_model.encode(batch_list[0], convert_to_tensor=convert_to_tensor)
			return embeddings
